...
3. Verifica que la categoria (bibl:, theo:, hist:, phil:) sea la correcta
...
**Error tipico:** Escribir `{ref:bibl:sermon-mount}` cuando la clave real es `theo:sermon-mount`.
**Error tipico:** Escribir `{term:Libre Albedrio}` en vez de `{term:libre-albedrio}`.
...
1. Se usa "vuestro/su" de maneras que marcan explicitamente otredad no-humana?
   - Mal: "vuestros teologos", "los minerales de vuestro planeta"
   - Bien: "los teologos han descubierto", "los minerales de este mundo"
...
| La Eleccion | — |
| Libre albedrio | — |
| El Creador / Dios | Deidad, Ser Supremo (contexto doctrinal permite "Dios") |
| Encarnacion | — |
...
- Citas biblicas textuales con capitulo:versiculo (ej: "Juan 3:16", "Mateo 5:3")
- Cualquier referencia directa a fuentes

### J) PARALELOS EVANGELICOS
Verificar que los ecos de los Evangelios en el texto son:
1. Organicos — surgen naturalmente del argumento, no insertados a la fuerza
2. Parafraseados — nunca citas textuales de la Biblia
3. Sin referencia — nunca incluyen capitulo:versiculo
4. Complementarios — muestran convergencia, no dependen del texto biblico
...
[Markup duplicado, terminologia incorrecta, atribuciones, citas biblicas]
...
- **Sin traduccion.** Doctrinas es un libro ES-only, no hay fases de traduccion.
